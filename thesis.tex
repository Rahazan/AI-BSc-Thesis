...
To do: \citep{brunken2003direct} is a good survey
How this cognitive load could be determined, i.e. by physiological measurement.

\section{Physiological response to cognitive load}
To do: How a human body responds to cognitive load physiologically, why this measurement method is a good idea. 

\subsection{Galvanic Skin Response}

Galvanic Skin Response (GSR) is an interesting physiological response to measure as sensors are cheap and simple, and the subject is not or marginally restricted in movement. Also, new devices such are smartwatches are fitted with GSR electrodes increasingly often.
...
This normalised GSR was then cut into the specific tasks. A task (slice of data) runs from the time that the first number is shown, to the time that an answer is given. These tasks were then smoothed one last time, to get rid of a small remaining amount of DC noise. This was again done using a Savitzky-Golay filter.
